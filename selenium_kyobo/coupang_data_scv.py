...
product_code = {
    'food': 194276,
    'household_items': 115673,
    'beauty': 176522,
    'interior': 184555,
    'electronics_digital': 178155,
    'home_kitchen': 185669,
    'maternity_baby': 221934,
    'pet_supplies': 115674,
    'toys': 317779,
    'automotive': 183960,
    'stationery_office': 177295,
    'sports': 317778,
    'books_music_movies': 317777,
    'health_supplements': 305798,
    'women_fashion': 186764,
    'men_fashion': 187069,
    'kids_baby_fashion': 508566,
    'unisex_clothing': 502993
}
...
for name, main_url in zip(original_list, url_list):
    ...
    for i in range(1, 31):
        temp_url = f"{main_url}?page={i}"
        driver.get(temp_url)
        time.sleep(5)
        print("*" * 10 + " " + str(i) + " Page start! " + "*" * 10)

        try:
            product_list_element = driver.find_elements(By.CLASS_NAME, "baby-product")
            for li in product_list_element:
                try:
                    # pid 추출
...
df = pandas.DataFrame(all_product_data, columns=[
    "product_id",
    "product_number",
    "category",
    "name",
